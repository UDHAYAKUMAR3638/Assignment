...
public class BlackJack1 implements card {
     ...
     static HashMap<Integer, Integer>map=new HashMap<>(){{
        put(2,4);//Space Complexity:O(n);
        put(3,4);// Time Complexity:O(1);
        put(4,4);
        put(5,4);
        put(6,4);
        put(7,4);
...
public int ranGen()//Space Complexity:O(n)
{
     int val=(int)Math.floor((Math.random()*(11-2+1)+2));
     if(map.get(val)>0)//Time Complexity:O(1);
     { map.put(val,map.get(val)-1);//Time Complexity:O(1);
     return val;
     }
     else
     { 
        while(map.get(val)==0)//Time Complexity:O(n);
...
public void choose()//Space Complexity:O(n)
{   
    if(curr){
    Scanner s=new Scanner(System.in);
...
public void playhit()//Space Complexity:O(n)
{  int val=ranGen();
   System.out.println("\nYou drew a "+val+".");
   player+=val;
   playert();
...
check for ending condition then again call choose method*/
public void dealerhit()//Space Complexity:O(n)
{  int val=ranGen();
  System.out.println("\nHe drews a "+val+".");
 dealer+=val;
  dealert();
...
public void stay()//Space Complexity:O(n)
{ int val=ranGen();
  System.out.println("\nOkay dealer's turn. ");
...
public void playert()//Space Complexity:O(n)
